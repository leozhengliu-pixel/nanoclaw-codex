...
4. `exec` the agent-runner inside that container
5. stream events back through the IPC directory
6. `rm -f` the container when the task completes or is cancelled
